...
class MovieViewTests(TestCase):
    ...
    def test_movie_list_view_search_and_filter(self):
        ...
        self.assertEqual(list(response.context["movies_list"]), [self.movie1])

        response = self.client.get(url, {"genre": [self.genre2.id]})
        self.assertEqual(list(response.context["movies_list"]), [self.movie2])
    ...
    def test_movie_detail_view_post_authenticated_rating_and_review(self):
        ...
        response = self.client.post(
            url, {"rating": 9, "text": "Amazing!", "is_review": "on"}
        )
        self.assertRedirects(response, url)

        self.assertTrue(
            UserRating.objects.filter(
                author=self.user, movie=self.movie1, value=9
            ).exists()
        )
        self.assertTrue(
